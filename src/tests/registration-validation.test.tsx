...
describe('RegistrationForm', () => {
  ...
  it('shows default address checkbox if "same as shipping" is checked', async () => {
    ...
    const billingCheckbox = screen.getByLabelText(/billing address same as shipping/i);
    fireEvent.click(billingCheckbox);
    expect(await screen.findByLabelText(/set as default address/i)).toBeInTheDocument();
  });
  //fist name
  it('Error: first name no longer than 20 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    const input = screen.getByLabelText('first name');
    fireEvent.change(input, { target: { value: '12345678901234567890q' } });
    await screen.findByText('First name must be no longer than 20 characters');
  });
  //last name
  it('Error: last name no longer than 20 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    const input = screen.getByLabelText('last name');
    fireEvent.change(input, { target: { value: '12345678901234567890qqqq' } });
    await screen.findByText('Last name must be no longer than 20 characters');
  });
  //age
  ...
  // street
  it('Error: street at least 5 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    const input = screen.getAllByLabelText(/street/i)[0];
    fireEvent.change(input, { target: { value: 'str' } });
    await screen.findByText('Street name must be at least 5 characters');
  });
  //city
  it('Error: city is at least 2 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    const input = screen.getAllByLabelText(/city/i)[0];
    fireEvent.change(input, { target: { value: 'a' } });
    await screen.findByText('City name must be at least 2 characters');
  });
  //postal code
  it('Error: city is at least 2 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    ...
    fireEvent.change(input, { target: { value: 'a' } });
    await screen.findByText('Postal code must be valid (e.g., 12345)');
  });
  it('Error: city is at least 2 characters', async (): Promise<void> => {
    render(<RegistrationForm />);
    const input = screen.getAllByLabelText(/postal code/i)[0];
  ...
  it('toggles submit button after filling form fields with valid data', async () => {
    ...
    const petDateOfBirthInput = screen.getByLabelText(/pet's date of birth/i);
