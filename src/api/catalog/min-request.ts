...
export async function getMinCost(): Promise<ProductProjection> {
  const token: string = await getCatalogToken();
  const url = `https://api.${API_CONFIG.region}.commercetools.com/${API_CONFIG.projectKey}/product-projections/search?sort=price asc&limit=1`;
  try {
...
export async function defineMinCost(): Promise<number> {
  const product = await getMinCost();
